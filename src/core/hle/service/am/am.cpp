...
void IStorageAccessor::Write(Kernel::HLERequestContext& ctx) {
    ...
    const std::size_t size{std::min(data.size(), (size_t)(backing.GetSize() - offset))};
...
void IStorageAccessor::Read(Kernel::HLERequestContext& ctx) {
    ...
    const u64 offset{rp.Pop<u64>()};
    const std::size_t size{std::min(ctx.GetWriteBufferSize(), (size_t)(backing.GetSize() - offset))};
